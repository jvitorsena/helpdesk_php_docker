...
if ($excluir){
  $sql = "DELETE FROM tbcategoria WHERE `tbcategoria`.`codigo_categoria` = {$excluir}";
  $resultado = mysqli_query($con, $sql);
  header('Location: listar_categorias.php?pesquisar=todos');
}
              ...
              <?php while ($linha = mysqli_fetch_assoc($resultado)) { ?>
                <div class="card mb-3 bg-light">
                  <div class="card-body">
                    ...
                    <?php echo $linha['nome_categoria'] . "  alterar  - <a href=listar_categorias.php?excluir=$codigo>Excluir</a> <br>"; ?>
                  </div>
                </div>
            <?php }
             ?>
